...
export default function FinanceWorkspace({
  ...
  const handleReconcile = async () => {
    ...
    if (!settlement?.id || !/^\d+$/.test(String(settlement.id))) {
      showToast('Select a persisted settlement before reconciliation');
      return;
    }
    try {
      const base = (import.meta.env.VITE_API_BASE_URL || '').replace(/\/$/, '');
      const res = await fetch(base + '/api/v1/finance/settlements/' + settlement.id + '/reconcile', { method: 'POST' });
      if (!res.ok) throw new Error('Reconciliation failed (' + res.status + ')');
      const data = await res.json();
      showToast('Reconciliation completed. Variance: ₹' + Number(data.variance || 0).toLocaleString('en-IN'));
    } catch (e) { showToast(e instanceof Error ? e.message : 'Reconciliation failed'); }
  };

  const handleFinanceReport = async (kind: 'summary' | 'tax') => {
    try {
      const base = (import.meta.env.VITE_API_BASE_URL || '').replace(/\/$/, '');
      const res = await fetch(base + '/api/v1/finance/reports/summary');
      if (!res.ok) throw new Error('Finance report failed (' + res.status + ')');
      const data = await res.json();
      const rows = Object.entries(data).map(([k,v]) => [k, v]);
      const csv = rows.map(r => r.join(',')).join('\n');
      const blob = new Blob([csv], { type: 'text/csv;charset=utf-8' });
      const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url;
      a.download = (kind === 'tax' ? 'GST_Tax_Summary_' : 'Financial_Report_') + Date.now() + '.csv'; a.click(); URL.revokeObjectURL(url);
      showToast(kind === 'tax' ? 'GST & tax summary downloaded' : 'Financial report downloaded');
    } catch (e) { showToast(e instanceof Error ? e.message : 'Finance report failed'); }
  };
    ...
    <div className="flex-1 overflow-y-auto bg-slate-50/50 p-4 sm:p-6 lg:p-7 space-y-6 max-w-[1720px] w-full mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <button
            onClick={() => handleFinanceReport('summary')}
        ...
        <div className="xl:col-span-4 space-y-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200/90 shadow-2xs p-5 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2.5">
              ...
              <button
                onClick={() => handleFinanceReport('tax')}
                className="flex items-center gap-2 p-2.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 transition-colors text-left"
              >
