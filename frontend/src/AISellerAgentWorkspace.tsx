...
export default function AISellerAgentWorkspace({ onClose }: Props) {
  ...
  React.useEffect(() => {
    ...
    api('/analytics/advanced')
      ...
      .then(data => { if (!cancelled && data) setAnalytics(data); })
      .catch(() => { /* analytics are enhancement-only; chat remains usable */ })
    ...
    <section className="ai-chat-window seller-os-window">
      <header className="ai-chat-header seller-os-header">
        ...
        <div className="ai-chat-header-actions">
          <button aria-label="Toggle insights" title="Toggle insights" onClick={() => setInsightsOpen(v => !v)}>{insightsOpen ? '⌃' : '⌄'}</button>
      ...
      {insightsOpen && <section className="seller-command-strip">
        ...
        <div className="seller-metrics">
          ...
          <Metric label="Inventory" value={analyticsLoading ? '—' : number(k.inventory_units)} detail={analytics?.forecasts?.inventory_days ? `${analytics.forecasts.inventory_days} days cover` : undefined} tone={analytics?.forecasts?.inventory_days !== undefined && analytics.forecasts.inventory_days < 7 ? 'danger' : ''} />
        </div>
      ...
      <footer className="ai-chat-composer seller-composer">
        <button className="composer-icon" aria-label="Emoji">☺</button>
        <textarea value={message} onChange={e => setMessage(e.target.value)} onKeyDown={handleKeyDown} placeholder="Ask your Seller Agent…" rows={1} disabled={chatting} />
