...
export default function AISellerAgentWorkspace({ onClose }: Props) {
  ...
  async function ask() {
    ...
    const priorConversation: ConversationItem[] = messages.slice(-12).map(item => ({
    ...
    try {
      const r = await api('/ai/seller-agent/chat', {
        method: 'POST',
        body: JSON.stringify({ message: text, conversation: priorConversation }),
      });
