...
def snapshot_listing(listing: Listing) -> dict[str, Any]:
    return {"id": listing.id, "product_id": listing.product_id, "marketplace_account_id": listing.marketplace_account_id, "sku": listing.sku, "external_listing_id": listing.external_listing_id, "status": listing.status, "title": listing.title, "price": str(listing.price) if listing.price is not None else None, "inventory_quantity": listing.inventory_quantity, "attributes": json.loads(listing.attributes_json or "{}"), "marketplace_data": json.loads(listing.marketplace_data_json or "{}"), "validation_errors": json.loads(listing.validation_errors_json or "[]")}


def validate_changes(changes: dict[str, Any]) -> dict[str, Any]:
    if not changes: raise ValueError("At least one listing field must be supplied")
    unknown = set(changes) - _ALLOWED_FIELDS - {key for key in changes if key.startswith("attribute:")}
    if unknown: raise ValueError(f"Unsupported listing fields: {', '.join(sorted(unknown))}")
    normalized: dict[str, Any] = {}
    for field, value in changes.items():
        if field == "title":
            if not isinstance(value, str) or not value.strip() or len(value) > 500: raise ValueError("title must be a non-empty string of at most 500 characters")
            normalized[field] = value.strip()
        elif field == "description":
            if not isinstance(value, str) or len(value) > 10000: raise ValueError("description must be a string of at most 10000 characters")
            normalized[field] = value
        elif field == "bullets":
            if not isinstance(value, list) or not value or len(value) > _MAX_BULLETS or not all(isinstance(item, str) and item.strip() for item in value): raise ValueError(f"bullets must contain 1-{_MAX_BULLETS} non-empty strings")
            normalized[field] = [item.strip() for item in value]
        elif field == "images":
            if not isinstance(value, list) or len(value) > _MAX_IMAGES or not all(isinstance(item, str) and item.strip() for item in value): raise ValueError(f"images must contain 0-{_MAX_IMAGES} non-empty URLs")
            normalized[field] = [item.strip() for item in value]
        elif field.startswith("attribute:"):
            name = field.split(":", 1)[1].strip()
            if not name or "/" in name or ".." in name: raise ValueError("attribute field name is invalid")
            normalized[field] = value
    return normalized
...
def create_listing_update(db: Session, *, seller_account_id: int, listing_id: int, changes: dict[str, Any], reason: str | None = None) -> ListingUpdate:
    ...
    db.add(update); db.commit(); db.refresh(update); return update
...
def reject_listing_update(db: Session, *, seller_account_id: int, update_id: int) -> ListingUpdate:
    ...
    update.status = ListingUpdateStatus.REJECTED.value; db.commit(); db.refresh(update); return update
...
def execute_listing_update(db: Session, *, seller_account_id: int, update_id: int) -> dict[str, Any]:
    ...
    attrs = json.loads(listing.attributes_json or "{}"); attrs.update(changes)
    if "title" in changes: listing.title = changes["title"]
    listing.attributes_json = _json(attrs); listing.marketplace_data_json = _json(result); listing.validation_errors_json = "[]"
    update.resulting_state_json = _json(snapshot_listing(listing)); update.status = ListingUpdateStatus.COMPLETED.value; update.completed_at = datetime.utcnow(); update.error = None
    db.commit(); return {"listing_update_id": update.id, "listing_id": listing.id, "sku": listing.sku, "marketplace": account.marketplace, "status": "completed", "changes": changes, "result": result}


def rollback_listing_update(db: Session, *, seller_account_id: int, update_id: int) -> ListingUpdate:
    original = db.scalar(select(ListingUpdate).where(ListingUpdate.id == update_id, ListingUpdate.seller_account_id == seller_account_id))
    if original is None: raise ValueError("Listing update not found")
    if original.status != ListingUpdateStatus.COMPLETED.value: raise ValueError("Only completed listing updates can be rolled back")
    previous = json.loads(original.previous_state_json or "{}"); proposed = json.loads(original.proposed_changes_json or "{}"); changes: dict[str, Any] = {}
    previous_attrs = previous.get("attributes") or {}
    for field in proposed:
        if field in {"title", "description", "bullets", "images"} and field in previous: changes[field] = previous[field]
        elif field.startswith("attribute:"):
            name = field.split(":", 1)[1]
            if name in previous_attrs: changes[field] = previous_attrs[name]
    if not changes: raise ValueError("No reversible fields found in this update")
    return create_listing_update(db, seller_account_id=seller_account_id, listing_id=original.listing_id, changes=changes, reason=f"Rollback of listing update #{original.id}")
