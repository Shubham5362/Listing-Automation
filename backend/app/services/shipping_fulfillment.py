...
def _seller(db: Session, user_id: int, seller_id: int) -> SellerAccount | None:
    return db.scalar(select(SellerAccount).where(SellerAccount.id == seller_id, SellerAccount.user_id == user_id))
...
def _read(db: Session, shipment: Shipment) -> dict:
    ...
    return {
        "id": shipment.id, "order_id": shipment.order_id, "marketplace_account_id": shipment.marketplace_account_id,
        "mode": shipment.mode, "status": shipment.status, "provider": shipment.provider, "awb": shipment.awb,
        "tracking_url": shipment.tracking_url, "label_url": shipment.label_url, "pickup_at": shipment.pickup_at,
        "shipped_at": shipment.shipped_at, "delivered_at": shipment.delivered_at, "last_synced_at": shipment.last_synced_at,
...
def create_shipment(db: Session, user_id: int, order_id: int, mode: FulfillmentMode, provider: str | None) -> Shipment:
    ...
    if existing:
        return existing
    row = Shipment(seller_account_id=order.seller_account_id, order_id=order.id, marketplace_account_id=order.marketplace_account_id, mode=mode.value, provider=provider)
    db.add(row); db.flush(); _event(db, row, row.status, "Fulfillment created", "system"); db.commit(); db.refresh(row)
    return row
...
def attach_label(db: Session, user_id: int, shipment_id: int, provider: str, awb: str | None, label_url: str | None, tracking_url: str | None) -> Shipment:
    ...
    if not row: raise ValueError("Shipment not found")
    ...
    row.provider = provider.strip(); row.awb = awb.strip() if awb else row.awb; row.label_url = label_url; row.tracking_url = tracking_url
    row.status = ShipmentStatus.LABEL_CREATED.value
    if row.awb:
        order = db.get(Order, row.order_id)
        if order: order.tracking_number = row.awb; order.carrier = row.provider
...
def update_status(db: Session, user_id: int, shipment_id: int, status: ShipmentStatus, message: str | None, awb: str | None, tracking_url: str | None) -> Shipment:
    ...
    if awb: row.awb = awb.strip()
    if tracking_url is not None: row.tracking_url = tracking_url
    if row.awb:
        order = db.get(Order, row.order_id)
        if order: order.tracking_number = row.awb; order.carrier = row.provider
    row.status = target
    ...
    if target == ShipmentStatus.SHIPPED.value:
        ...
        if order and order.status == OrderStatus.PACKED.value:
            order.status = OrderStatus.SHIPPED.value; order.shipped_at = now
    elif target == ShipmentStatus.DELIVERED.value:
        row.delivered_at = now
        order = db.get(Order, row.order_id)
        if order and order.status == OrderStatus.SHIPPED.value:
            order.status = OrderStatus.DELIVERED.value; order.delivered_at = now
...
def list_shipments(db: Session, user_id: int, status: ShipmentStatus | None, limit: int) -> list[dict]:
    ...
    if status: stmt = stmt.where(Shipment.status == status.value)
...
def provider_capabilities(marketplace: str) -> dict:
    # Live marketplace write operations remain explicitly capability-gated; no fake labels/AWBs are generated.
    return {"marketplace": marketplace, "live_label_api": False, "live_tracking_sync": False, "manual_awb_supported": True, "note": "Record verified provider output or connect a shipping provider before live label/tracking automation."}
