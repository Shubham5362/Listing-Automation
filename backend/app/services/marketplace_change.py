...
def _fields(schema_json: str) -> dict[str, dict[str, Any]]:
    try:
        payload = json.loads(schema_json or "{}")
...
def _severity(change: dict[str, Any]) -> str:
    if change["change_type"] in {"FIELD_REQUIRED_CHANGED", "FIELD_REMOVED", "FIELD_TYPE_CHANGED"}:
        return "high"
    if change["change_type"] in {"FIELD_OPTIONS_CHANGED", "UNIT_CHANGED", "FIELD_RENAMED"}:
        return "medium"
    return "low"
...
def scan_marketplace(db: Session, marketplace: str, category: str | None = None) -> dict[str, Any]:
    ...
    for schema in schemas:
        ...
        if latest and _fingerprint(json.loads(latest.schema_json)) == current_fp:
            result["categories"].append({"category": schema.category, "status": "unchanged", "snapshot_id": latest.id})
            continue
...
def _record_impact(db: Session, change: MarketplaceSchemaChange, detail: dict[str, Any]) -> None:
    products = db.scalars(select(Product).where(Product.category == change.category)).all()
    canonical = detail.get("canonical")
    affected = 0
    for product in products:
        if canonical in {"SKU", "TITLE", "BRAND", "CATEGORY", "HSN", "GST_RATE"}:
            value = getattr(product, {"SKU": "sku", "TITLE": "title", "BRAND": "brand", "CATEGORY": "category", "HSN": "hsn_code", "GST_RATE": "gst_rate"}[canonical], None)
        ...
        if has_value or detail["change_type"] in {"FIELD_ADDED", "FIELD_REQUIRED_CHANGED", "FIELD_REMOVED"}:
            affected += 1
    ...
    review = affected if not change.auto_adaptable else 0
    auto_fix = affected if change.auto_adaptable else 0
    blocked = affected if detail["change_type"] in {"FIELD_REMOVED", "FIELD_TYPE_CHANGED"} else 0
...
def apply_change(db: Session, change_id: int) -> dict[str, Any]:
    ...
    new_field = json.loads(row.new_value_json or "null")
    marketplace_field = new_field.get("name") if isinstance(new_field, dict) else row.field_name
    ...
    db.add(mapping); row.status = "validated"; db.commit()
    return {"change_id": row.id, "mapping_version": mapping.version, "marketplace_field": mapping.marketplace_field, "status": row.status}
