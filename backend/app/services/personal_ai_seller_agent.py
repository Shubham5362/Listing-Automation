...
class PersonalAISellerAgentService:
    ...
    This first agent layer intentionally uses deterministic business intelligence rather
    than inventing LLM output. A future LLM can consume this structured context safely.
    """
    ...
    def context(self) -> dict[str, Any]:
        ...
        return {
            ...
            "active_listings": sum(l.status == "active" for l in listings),
            "inventory_items": len(inventory),
            "inventory_units": sum(max(i.quantity - i.reserved_quantity, 0) for i in inventory),
            "low_stock": sum(max(i.quantity - i.reserved_quantity, 0) <= i.reorder_level for i in inventory if i.reorder_level > 0),
            "out_of_stock": sum(max(i.quantity - i.reserved_quantity, 0) <= 0 for i in inventory),
    ...
    def inventory_issues(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        for item in items:
            ...
            if available <= 0:
                severity = "critical"
            elif item.reorder_level > 0 and available <= item.reorder_level:
                severity = "high"
    ...
    def advertising_issues(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        for campaign in campaigns:
            ...
            sales = sum(float(r.sales) for r in rows)
            ...
            waste = min(100.0, spend / max(sales, 1.0) * 100) if sales <= 0 or (acos is not None and acos > 50) else 0.0
    ...
    def brief(self) -> dict[str, Any]:
        ...
        critical = sum(r["priority"] == "critical" for r in recommendations)
    ...
    def chat(self, message: str, create_plan: bool = False) -> dict[str, Any]:
        ...
        if any(word in text for word in ("stock", "inventory", "out of stock", "replenish")):
            focus = self.inventory_issues(10)
            intent = "inventory"
        elif any(word in text for word in ("ad", "advertising", "campaign", "acos", "roas")):
            focus = self.advertising_issues(10)
            intent = "advertising"
        elif any(word in text for word in ("price", "pricing", "margin", "competitor")):
            focus = self.pricing_opportunities(10)
            intent = "pricing"
        else:
            focus = self.recommendations(10)
            intent = "business_health"
        plan = [{"step": 1, "action": "review verified Seller Hub data", "status": "ready"}, {"step": 2, "action": f"evaluate {intent} priorities", "status": "ready"}, {"step": 3, "action": "request approval before marketplace writes", "status": "required"}]
        created_actions: list[dict[str, Any]] = []
        if create_plan and focus:
            for item in focus[:5]:
                action_name = "marketplace_operation" if intent in {"inventory", "pricing"} else "automation_run"
                action = create_personal_action(self.db, action=action_name, payload={"source": "ai_seller_agent", "intent": intent, "recommendation": item}, reason=f"AI Seller Agent plan from request: {message[:300]}")
                created_actions.append({"id": action.id, "action": action.action, "risk": action.risk, "status": action.status})
        response = {"agent": "personal_ai_seller_agent", "intent": intent, "message": message, "answer": self._answer(intent, focus), "plan": plan, "recommendations": focus, "approval_required": True, "created_actions": created_actions, "execution": "approved actions are queued through the existing Action Control pipeline"}
        self.db.add(AuditLog(action="ai_agent.chat", resource_type="ai_agent", resource_id=None, details=f'{{"intent":"{intent}","create_plan":{str(create_plan).lower()}}}'))
        self.db.commit()
        return response
    ...
    def _answer(intent: str, rows: list[dict[str, Any]]) -> str:
        if not rows:
            return f"No {intent} issue requiring attention was found in the available live data."
