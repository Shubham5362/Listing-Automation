...
from app.models.core import AuditLog, Job, SellerAccount
from app.services.action_control import approve_personal_action, create_personal_action, reject_personal_action
from app.services.jobs import mark_job_finished, retry_job
...
def _job_dict(job: Job) -> dict:
    return {
        "id": job.id, "name": job.name, "status": job.status, "seller_account_id": job.seller_account_id,
        "attempts": job.attempts, "max_attempts": job.max_attempts, "error": job.error,
        "result": json.loads(job.result) if job.result else None, "created_at": _dt(job.created_at),
        "started_at": _dt(job.started_at), "finished_at": _dt(job.finished_at), "run_after": _dt(job.run_after),
    }


def _action_dict(item: ActionRequest) -> dict:
    return {
        "id": item.id, "action": item.action, "risk": item.risk, "status": item.status,
        "payload": json.loads(item.payload) if item.payload else {}, "reason": item.reason, "job_id": item.job_id,
        "result": json.loads(item.result) if item.result else None, "error": item.error,
        "created_at": _dt(item.created_at), "approved_at": _dt(item.approved_at),
        "rejected_at": _dt(item.rejected_at), "completed_at": _dt(item.completed_at),
    }
...
def request_personal_action(request: PersonalActionRequest, db: Session = Depends(get_db)) -> dict:
    try:
        item = create_personal_action(db, action=request.action, payload=request.payload, reason=request.reason)
...
def personal_audit(db: Session = Depends(get_db), limit: int = Query(default=100, ge=1, le=100)) -> list[dict]:
    seller_id = _seller(db)
    resource_ids = select(ActionRequest.id).where(ActionRequest.seller_account_id == seller_id)
    rows = db.scalars(select(AuditLog).where((AuditLog.resource_type == "action_request", AuditLog.resource_id.in_(select(resource_ids.scalar_subquery()))) | (AuditLog.action.in_(["action.requested", "action.approved", "action.rejected"])))).order_by(AuditLog.created_at.desc()).limit(limit)).all()
