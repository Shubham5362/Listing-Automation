...
from app.schemas.returns_ai import ReturnAIRead, ReturnAIRequest, SupportAIRead, SupportAIRequest
from app.services.returns_ai import ReturnsSupportAIService

router = APIRouter(prefix="/returns", tags=["returns"])

def _seller_ids(db: Session, user: User) -> list[int]: return list(db.scalars(select(SellerAccount.id).where(SellerAccount.user_id == user.id)).all())
...
def create_return(payload: ReturnCreate, user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> ReturnRead:
    ...
    if payload.order_item_id and not db.scalar(select(OrderItem).where(OrderItem.id == payload.order_item_id, OrderItem.order_id == order.id)): raise HTTPException(status_code=400, detail="Order item does not belong to order")
    data = payload.model_dump(); data.pop("order_id", None); row = ReturnRequest(seller_account_id=order.seller_account_id, order_id=order.id, **data)
    db.add(row); db.commit(); db.refresh(row); return _return_read(row)
...
@router.post("/ai/analyze", response_model=ReturnAIRead)
def analyze_return_ai(payload: ReturnAIRequest, user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> ReturnAIRead:
    if not _seller_ids(db, user): raise HTTPException(status_code=400, detail="Seller account not found")
    return ReturnAIRead.model_validate(ReturnsSupportAIService.analyze_return(payload.reason, payload.customer_note, payload.refund_amount).__dict__)

@router.post("/ai/support", response_model=SupportAIRead)
def analyze_support_ai(payload: SupportAIRequest, user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> SupportAIRead:
    if not _seller_ids(db, user): raise HTTPException(status_code=400, detail="Seller account not found")
    result = ReturnsSupportAIService.analyze_support(payload.subject, payload.message, payload.priority, payload.customer_name, payload.tone, payload.language)
    return SupportAIRead.model_validate(result.__dict__)
...
def update_return(return_id: int, payload: ReturnStatusUpdate, user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> ReturnRead:
    ...
    db.commit(); db.refresh(row); return _return_read(row)
...
def create_issue(payload: CustomerIssueCreate, user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> CustomerIssueRead:
    ...
    row = CustomerIssue(seller_account_id=seller_id, **payload.model_dump()); db.add(row); db.commit(); db.refresh(row); return _issue_read(row)
...
def update_issue(issue_id: int, payload: CustomerIssueUpdate, user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> CustomerIssueRead:
    sellers = _seller_ids(db, user); row = db.scalar(select(CustomerIssue).where(CustomerIssue.id == issue_id, CustomerIssue.seller_account_id.in_(sellers))) if sellers else None
    ...
    db.commit(); db.refresh(row); return _issue_read(row)
...
def suggest_reply(issue_id: int, payload: AIReplyRequest, user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> CustomerIssueRead:
    sellers = _seller_ids(db, user); row = db.scalar(select(CustomerIssue).where(CustomerIssue.id == issue_id, CustomerIssue.seller_account_id.in_(sellers))) if sellers else None
    if not row: raise HTTPException(status_code=404, detail="Customer issue not found")
    intelligence = ReturnsSupportAIService.analyze_support(row.subject, row.message, row.priority, row.customer_name, payload.tone)
    row.ai_reply_suggestion = intelligence.reply
    if intelligence.priority in {"high", "urgent"} and row.status == CustomerIssueStatus.OPEN.value: row.priority = intelligence.priority
    db.commit(); db.refresh(row); return _issue_read(row)
