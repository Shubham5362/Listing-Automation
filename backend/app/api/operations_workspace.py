...
def overview(db: Session = Depends(get_db)) -> dict:
    """Read-only personal Seller Hub overview.

    Phase 1 intentionally has no login/auth gate: the private deployment is
    the access boundary. Marketplace credentials and write operations remain
    behind their dedicated APIs and are not exposed by this endpoint.
    """
    seller_ids = list(db.scalars(select(SellerAccount.id)).all())
    seller_filter = SellerAccount.id.in_(seller_ids) if seller_ids else None
    ...
    order_rows = db.execute(
        ...
        .where(Order.seller_account_id.in_(seller_ids))
        .group_by(Order.status)
    ).all()
    orders_total = sum(count for _, count in order_rows)
    ...
    job_rows = db.execute(
        ...
        .where(Job.seller_account_id.in_(seller_ids))
        .group_by(Job.status)
    ).all()
    ...
    accounts = list(
        db.scalars(select(MarketplaceAccount).where(MarketplaceAccount.seller_account_id.in_(seller_ids))).all()
    )
    marketplaces = [
        {
            "id": account.id,
            "marketplace": account.marketplace,
            "status": "connected" if account.is_active else "disabled",
    ...
    if failed_jobs:
        alerts.append({"type": "jobs", "severity": "critical", "message": "Background jobs failed", "count": failed_jobs})
    disconnected = sum(1 for account in accounts if not account.is_active or account.last_error)
    if disconnected:
        alerts.append({"type": "marketplace", "severity": "warning", "message": "Marketplace connection needs attention", "count": disconnected})

    activity = [
        {
            "type": "job",
            "title": f"{job.name} · {job.status}",
            "status": job.status,
    ...
    return {
        ...
        "jobs": {"by_status": job_counts, "recent": activity},
