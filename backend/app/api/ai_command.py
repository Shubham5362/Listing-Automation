...
def _read(command) -> AICommandRead:
    ...
    return AICommandRead(
        id=command.id, query=command.query, intent=command.intent, status=command.status,
        trace_id=command.trace_id, answer=response.get("answer", ""),
        evidence=response.get("evidence", []), recommendations=response.get("recommendations", []),
        actions=response.get("actions", []), created_at=command.created_at.isoformat(),
    )
...
def execute_command(payload: AICommandCreate, seller_account_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> AICommandRead:
    ...
    return _read(run_command(db, user, seller_account_id, payload))


@router.post("/{command_id}/approve", response_model=AICommandRead)
def approve_command(command_id: int, seller_account_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)) -> AICommandRead:
    _seller(db, user, seller_account_id)
    try:
        command = AICommandExecutionService(db, user).approve_and_execute(command_id, seller_account_id)
