...
@router.post("/policy")
def set_policy(payload: PolicyIn, db: Session = Depends(get_db)):
...
def create_workflow(payload: WorkflowIn, db: Session = Depends(get_db)):
    try:
        run = start_workflow(db, seller_id(), payload.workflow_key, payload.idempotency_key)
        db.commit()
...
def finish_workflow(run_id: int, payload: CompleteIn, db: Session = Depends(get_db)):
    from app.models.production import AutonomousWorkflowRun
    run = db.get(AutonomousWorkflowRun, run_id)
    if not run or run.seller_account_id != seller_id():
