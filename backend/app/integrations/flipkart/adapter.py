...
from app.integrations.base import InventoryItem, MarketplaceAccountContext, MarketplaceClient, MarketplaceIntegrationError, MarketplaceOperationUnsupported, MarketplaceOrder, MarketplaceProduct, PriceQuote
...
class FlipkartSellerApiAdapter(MarketplaceClient):
    ...
    def list_products(self, account: MarketplaceAccountContext, *, limit: int = 50) -> list[MarketplaceProduct]:
        target = max(1, min(limit, 100)); products: list[MarketplaceProduct] = []; batch = 0
        while len(products) < target:
            ...
            if not data.get("hasMore", False) or not rows: break
            batch += 1
    ...
    def list_orders(self, account: MarketplaceAccountContext, *, limit: int = 50) -> list[MarketplaceOrder]:
        ...
        rows = data.get("shipments", data.get("orderItems", [])); result: list[MarketplaceOrder] = []
    ...
    def get_inventory(self, account: MarketplaceAccountContext, *, skus: list[str] | None = None) -> list[InventoryItem]:
        selected = skus or [product.sku for product in self.list_products(account, limit=100)]; result: list[InventoryItem] = []
    ...
    def get_prices(self, account: MarketplaceAccountContext, *, skus: list[str]) -> list[PriceQuote]:
        ...
        for start in range(0, min(len(skus), 100), 10):
            ...
            for sku, listing in details.items():
                attrs = listing.get("attributeValues", listing) if isinstance(listing, dict) else {}; raw = attrs.get("selling_price")
                if raw is not None: result.append(PriceQuote(sku=str(sku), price=Decimal(str(raw)), currency="INR"))
        return result

    def _product_id(self, sku: str) -> str:
        listing = self._listing_details([sku]).get(sku, {}); product_id = listing.get("productId") or listing.get("product_id")
        if not product_id: raise MarketplaceIntegrationError(f"Flipkart product ID not found for SKU '{sku}'")
    ...
    def update_price(self, account: MarketplaceAccountContext, *, sku: str, price: Decimal) -> None:
        ...
        details = self._listing_details([sku]); listing = details.get(sku, {}); attrs = listing.get("attributeValues", listing) if isinstance(listing, dict) else {}; mrp = Decimal(str(attrs.get("mrp", price)))
    ...
    def update_listing(self, account: MarketplaceAccountContext, *, sku: str, changes: dict[str, Any]) -> dict[str, Any]:
        raise MarketplaceOperationUnsupported("Flipkart listing update is not implemented by the current verified adapter; no marketplace write is attempted")

    def fetch_report(self, account: MarketplaceAccountContext, report_type: str) -> dict[str, Any]:
        if not report_type.strip(): raise ValueError("report_type must not be empty")
