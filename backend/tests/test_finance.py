...
def test_finance_entry_and_settlement_reconciliation() -> None:
    with TestClient(app) as client:
        email = f"finance-{uuid4().hex}@example.com"
        ...
        settlement = client.post("/api/v1/finance/settlements", headers=headers, json={"marketplace_account_id": account_id, "external_settlement_id": f"SET-{uuid4().hex}", "period_start": start, "period_end": end, "gross_amount": 1200, "fees_amount": 100, "refunds_amount": 50, "net_amount": 1050})
...
def test_finance_isolation() -> None:
    with TestClient(app) as client:
        ...
        owner_a, account_a = create(f"finance-a-{uuid4().hex}@example.com", "Seller A")
        owner_b, _ = create(f"finance-b-{uuid4().hex}@example.com", "Seller B")
